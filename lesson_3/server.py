...
def process_message(message, names, listen_socks):
    ...
    if message[DESTINATION] in names and names[message[DESTINATION]] in listen_socks:
        ...
        LOGGER.info(f"Отправлено сообщение пользователю {message[DESTINATION]} " f"от пользователя {message[SENDER]}.")
    elif message[DESTINATION] in names and names[message[DESTINATION]] not in listen_socks:
        raise ConnectionError
    else:
        LOGGER.error(
            f"Пользователь {message[DESTINATION]} не зарегистрирован на сервере, " f"отправка сообщения невозможна."
        )
        ...
        if recv_data_lst:
            for client_with_message in recv_data_lst:
                try:
                    ...
                    LOGGER.info(f"Клиент {client_with_message.getpeername()} " f"отключился от сервера.")
